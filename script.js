...
let category="All",search="",sort="default",favs=load("gx_favs",[]),history=load("gx_history",[]),fb=null,msgUnsub=null,presUnsub=null,presRef=null,presStop=null,currentGame=null;
...
function closeModals(){if($("#gameModal").classList.contains("show"))closeGame();["settingsModal","infoModal","feedbackModal","reportModal","communityModal"].forEach(id=>$("#"+id).classList.remove("show"));document.body.style.overflow="";stopChat()}
...
async function openCommunity(){closeSidebar();$("#communityModal").classList.add("show");document.body.style.overflow="hidden";$("#communityError").textContent="";const g=guest();if(!g){$("#guestGate").classList.remove("hidden");$("#chatArea").classList.add("hidden");return}$("#guestGate").classList.add("hidden");$("#chatArea").classList.remove("hidden");await startChat(g)}
function closeCommunity(){$("#communityModal").classList.remove("show");document.body.style.overflow="";stopChat()}
async function joinCommunity(){const name=$("#guestName").value.trim(),u0=$("#guestUsername").value.trim();$("#communityError").textContent="";if(name.length<2||!/^@?[A-Za-z0-9_]{3,20}$/.test(u0)){$("#communityError").textContent="Enter a valid name and username.";return}const user=await ensureAuth();if(!user){$("#communityError").textContent="Firebase Authentication unavailable. Enable Anonymous sign-in in Firebase Console.";return}const username=u0.startsWith("@")?u0:"@"+u0;const g={name,username,uid:user.uid};save("gx_guest",g);$("#guestGate").classList.add("hidden");$("#chatArea").classList.remove("hidden");await startChat(g);tap()}
async function startChat(g){stopChat();const user=await ensureAuth();if(!user){setChatStatus("Firebase Authentication unavailable","err");return}g.uid=user.uid;save("gx_guest",g);try{const db=fb.getDatabase(fb.app),ref=fb.ref(db,"community/messages"),q=fb.query(ref,fb.orderByChild("createdAt"),fb.limitToLast(100));setChatStatus("Connecting…");msgUnsub=fb.onValue(q,snap=>{const a=[];snap.forEach(x=>a.push({id:x.key,...(x.val()||{})}));a.sort((x,y)=>(Number(x.createdAt)||0)-(Number(y.createdAt)||0));renderMessages(a,g);setChatStatus(`${a.length} message${a.length!==1?"s":""} • Connected`,"ok")},err=>{console.error("Chat read error",err);setChatStatus("Chat read failed — check Realtime Database Rules","err")});const input=$("#chatInput");if(input){input.disabled=false;input.focus()}}catch(e){console.error(e);setChatStatus("Chat unavailable — check Firebase Database","err")}}
function stopChat(){if(msgUnsub){try{msgUnsub()}catch{}msgUnsub=null}}
function renderMessages(a,g){const el=$("#messages");if(!el)return;if(!a.length){el.innerHTML='<div class="chat-empty"><i class="fas fa-comments"></i><b>No messages yet</b><span>Be the first to say hello 👋</span></div>';return}el.innerHTML=a.map(m=>`<div class="msg ${m.uid===g.uid?"me":""}"><b>${esc(m.name||"Guest")} ${esc(m.username||"")}</b><div class="bubble">${esc(m.text||"")}</div><small>${new Date(Number(m.createdAt)||Date.now()).toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"})}</small></div>`).join("");el.scrollTop=el.scrollHeight}
async function sendMessage(){const i=$("#chatInput"),btn=$("#sendChatBtn"),t=i?.value.trim(),g=guest();if(!i||!t||!g)return;const user=await ensureAuth();if(!user){setChatStatus("Please reconnect to Community","err");return}if(btn){btn.disabled=true;btn.innerHTML='<i class="fas fa-spinner fa-spin"></i>'}try{const db=fb.getDatabase(fb.app),ref=fb.push(fb.ref(db,"community/messages"));await fb.set(ref,{uid:user.uid,name:g.name,username:g.username,text:t.slice(0,500),createdAt:Date.now()});i.value="";setChatStatus("Sent ✓ • Waiting for realtime update","ok");tap()}catch(e){console.error("Send message error",e);setChatStatus("Message failed — check Realtime Database Rules","err");toast("Message failed","err")}finally{if(btn){btn.disabled=false;btn.innerHTML='<i class="fas fa-paper-plane"></i>'}i.focus()}}
function leaveCommunity(){stopChat();localStorage.removeItem("gx_guest");$("#guestGate").classList.remove("hidden");$("#chatArea").classList.add("hidden");$("#chatInput").value="";setChatStatus("");toast("Guest profile removed")}
